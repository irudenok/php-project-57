                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-center font-medium space-x-3">
                                    ...
                                    <form id="delete-status-form-{{ $status->id }}" method="POST" action="{{ route('task_statuses.destroy', $status) }}" style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                    <a href="{{ route('task_statuses.destroy', $status) }}"
                                       onclick="event.preventDefault(); if(confirm('Вы уверены, что хотите удалить этот статус?')) { document.getElementById('delete-status-form-{{ $status->id }}').submit(); }"
